...
- **HTTPS/ACM certificate for the ALB.** The API currently serves plain HTTP. Adding TLS needs a domain the ALB's DNS can point at and an ACM certificate for it — a decision only the project owner can make (does the team own a domain, and which one?). Flagged by Plan 2's final review as a real gap for a "production-ready" API serving a mobile client, but deliberately not fixed as part of that review's fix wave — it needs input this spec can't supply on its own. Follow-up once a domain decision is made.
- **`scripts/seed_catalog.py` has no network path to RDS.** RDS sits in a private subnet by design (Component: Data warehouse), reachable only from inside the VPC. `seed_catalog.py` as built runs from a developer's local machine, which can't reach it. Flagged by Plan 2's final review; ruled out of that review's fix wave because the real fix (running the seeder as its own in-VPC ECS task, similar to the index-builder task) is new infrastructure, not a bug fix. This is a load-bearing follow-up — catalog seeding will not actually work end-to-end until it's addressed — worth its own small plan once Plan 3 (backend integration) lands, or folded into Plan 3 if the sequencing makes more sense there.
