...
BASE_URL = "https://carrotegypt.com/en"
API = f"{BASE_URL}/wp-json/wc/store/v1"
HEADERS = {"User-Agent": "Mozilla/5.0", "Accept": "application/json"}
CATEGORIES = ["girls", "boys", "baby-girl", "baby-boy", "baby"]
    ...
    for slug in CATEGORIES:
        ...
        while True:
            for attempt in range(3):
                try:
                    r = session.get(f"{API}/products", params={"category": cat_id, "per_page": 100, "page": page}, timeout=60)
                    break
                except requests.exceptions.RequestException as e:
                    print(f"  Request failed ({e}); retry {attempt + 1}/3")
                    time.sleep(2)
            else:
                print("  Giving up on this category after 3 failed attempts")
                break
            if not r.ok:
                break
            products = r.json()
