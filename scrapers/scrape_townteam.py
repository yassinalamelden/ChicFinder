...
def main():
    ...
    output = "data/raw/townteam.jsonl"
    session = requests.Session()
    session.headers.update(HEADERS)

    print("Fetching Town Team products...")
    all_products = []
    url = f"{API}?limit=250"
    ...
    for p in all_products:
        ...
        rec = {
            ...
            "product_id": pid,
            "title": title,
            "brand": strip(p.get("vendor")) or "Town Team",
            "category": strip(p.get("product_type")) or None,
