...
BASE_URL = "https://or-egypt.com"
API = f"{BASE_URL}/products.json"
HEADERS = {"User-Agent": "Mozilla/5.0", "Accept": "application/json"}
GENDER_TAGS = {"men", "kids", "boys", "girls", "unisex", "women", "special sizes"}
# Maps a raw Shopify tag (lowercased) to the 3-way gender scheme the rest of
# the dataset uses (Men/Women/Kids). OR tags products cleanly per-item (verified:
# some pages are 100% "Women", others 100% "Men") -- brand-level default would
# be wrong here, unlike the single-gender brands.
GENDER_MAP = {"men": "Men", "women": "Women", "kids": "Kids", "boys": "Kids", "girls": "Kids"}
...
def main():
    ...
    output = "data/raw/or.jsonl"
    session = requests.Session()
    session.headers.update(HEADERS)

    print("Fetching OR products...")
    all_products = []
    page = 1
    ...
    for p in all_products:
        ...
        gender = next((GENDER_MAP[t.lower()] for t in tags if t.lower() in GENDER_MAP), None)

        rec = {
            "source": "or",
            "product_id": pid,
            "title": title,
            "brand": strip(p.get("vendor")) or "OR",
            "category": category,
